...
export async function runDiscoveryForUser(db: VantageClient, userId: string): Promise<{ suggestionsFound: number }> {
  ...
  const { data: existingSources } = await db.from("sources").select("name, handle").eq("user_id", userId);
  ...
  for (const topic of ranked) {
    ...
    const rows = suggestions.map((s) => ({
      ...
      name: s.name,
      handle: s.handle,
      platform: s.platform as Database["vantage"]["Tables"]["source_suggestions"]["Row"]["platform"],
      url: s.url,
      reason: s.reason,
